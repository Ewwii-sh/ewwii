...
pub fn handle_nbcl_err(err: NbclError, code: &str, file_id: Option<&str>) -> String {
    match err {
        NbclError::Parse { message, hint, span } =>
            format_codespan_error(code, message, hint, span.unwrap_or(Span::dummy()), file_id),
        NbclError::Ast { message, hint, span } =>
            format_codespan_error(code, message, hint, span.unwrap_or(Span::dummy()), file_id),
        NbclError::IO { message, hint, path: _ } =>
            format_codespan_error(code, message, hint, Span::dummy(), file_id),
...
fn format_codespan_error(
    ...
    let diagnostic =
        Diagnostic::error().with_message(&message).with_labels(labels).with_notes(notes);
