...
use super::{
    file_provider::{FilesError, DuckFileProvider},
...
impl Config {
    fn append_toplevel(&mut self, files: &mut impl DuckFileProvider, toplevel: TopLevel) -> DiagResult<()> {
        match toplevel {
            ...
            TopLevel::Include(include) => {
                let (_, toplevels) = files.load_duck_file(PathBuf::from(&include.path)).map_err(|err| match err {
    ...
    pub fn generate(files: &mut impl DuckFileProvider, elements: Vec<Ast>) -> DiagResult<Self> {
        let mut config = Self {
    ...
    pub fn generate_from_main_file(files: &mut impl DuckFileProvider, path: impl AsRef<Path>) -> DiagResult<Self> {
        let (_span, top_levels) = files.load_duck_file(path.as_ref().to_path_buf()).map_err(|err| match err {
